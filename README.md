...
- Concurrent duplicates. ClaimService performs an initial application-level check for an existing claim. Because two requests can pass that check concurrently, the database unique constraint on idempotency_key provides the final concurrency guard. The sample includes handling for a losing duplicate insert, while production behaviour should be validated against the selected database and transaction isolation strategy. ClaimStatePersisterTest verifies the application-level duplicate-insert handling.
...
Once `retryCount` reaches `claims.retry.max-attempts`, the claim moves to `MANUAL_REVIEW` instead of retrying again. This demonstrates locally the operational outcome that a production SQS DLQ and alerting flow would support.
